  ...
  match: router({
    ...
    retrySettlement: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 兼容历史误标：status=settled 但没有微信单号的记录允许重新打款
        if (settlement.status === 'settled' && settlement.wxBatchId) throw new TRPCError({ code: 'BAD_REQUEST', message: '已结算' });
        ...
        try {
          if (settlement.wxBatchId) {
            ...
            try {
              ...
              console.warn(`[retrySettlement] query failed match=${input.matchId} bill=${existingBillNo}:`, queryErr?.message || queryErr);
            }
          }
