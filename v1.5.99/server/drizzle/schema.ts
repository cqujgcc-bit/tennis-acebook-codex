...
export const matchParticipants = mysqlTable("match_participants", {
  ...
  matchUserUnique: uniqueIndex("match_participants_match_user_unique").on(table.matchId, table.userId),
}));
...
// ─── Match Settlement Transfers（结算分笔转账）──────────────────────────────
// 微信商家转账单笔上限 200 元。每个分笔、每次尝试均保留独立记录，避免重试时重复付款。
export const matchSettlementTransfers = mysqlTable("match_settlement_transfers", {
  id: int("id").autoincrement().primaryKey(),
  matchId: int("matchId").notNull(),
  sequence: int("sequence").notNull(),
  attempt: int("attempt").default(1).notNull(),
  amountFen: int("amountFen").notNull(),
  outBillNo: varchar("outBillNo", { length: 32 }).notNull().unique(),
  ...
  matchSequenceAttemptUnique: uniqueIndex("match_settlement_transfers_match_sequence_attempt_unique")
    .on(table.matchId, table.sequence, table.attempt),
}));
export type MatchSettlementTransfer = typeof matchSettlementTransfers.$inferSelect;
