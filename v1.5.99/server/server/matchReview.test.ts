...
describe("matchReview", () => {
  ...
  it("match.create rejects a start time in the past", async () => {
    const c = caller(1);
    await expect(
      c.match.create({
        title: "Past Match",
    ...
    ).rejects.toMatchObject({ code: "BAD_REQUEST", message: "球局开始时间必须晚于当前时间" });
  });

  it("match.create rejects an end time before the start time", async () => {
    const c = caller(1);
    await expect(
      c.match.create({
    ...
    ).rejects.toMatchObject({ code: "BAD_REQUEST", message: "结束时间必须晚于开始时间" });
  });

  it("coach public list is disabled for this release", async () => {
    const c = caller(null);
    await expect(c.coach.list()).resolves.toEqual([]);
  });
});
