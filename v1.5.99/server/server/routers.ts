...
import { buildClientPaymentParams, buildMatchTransferRemark, createPrepay, refundOrder, transferToUser, queryTransferBill, cancelTransferBill, encodeTransferPackageInfo, decodeTransferPackageInfo, generateOrderId, isWxpayConfigured, wxpayConfig, isValidOutBillNo } from "./wxpay";
import { MAX_SINGLE_TRANSFER_FEN, hasSplitSettlementPlan, processSplitSettlementTransfer, syncSplitSettlementTransfer } from "./settlement-transfer";
...
const COACH_PUBLIC_FEATURE_ENABLED = false;

function chinaDateString(now = new Date()): string {
  return new Date(now.getTime() + 8 * 3600 * 1000).toISOString().slice(0, 10);
}

function matchStartTimestamp(match: { matchDate?: string | null; startTime?: string | null }): number {
  if (!match.matchDate || !match.startTime) return Number.POSITIVE_INFINITY;
  return new Date(`${String(match.matchDate).slice(0, 10)}T${match.startTime}:00+08:00`).getTime();
}

function publicDisplayName(name?: string | null): string | null {
  const value = (name || '').trim();
  if (!value) return null;
  return /^1[3-9]\d{9}$/.test(value) ? '球友' : value;
}
...
export const appRouter = router({
  ...
  auth: router({
    ...
    wxpayDebug: adminProcedure
      .mutation(async () => {
        ...
        return {
          ...
          transferSceneId: wxpayConfig.transferSceneId || 'EMPTY',
          transferNotifyUrl: wxpayConfig.transferNotifyUrl || 'EMPTY',
          transferSceneInfoType: wxpayConfig.transferSceneInfoType || 'EMPTY',
    ...
    debugSettlement: adminProcedure
      .input(z.object({ matchId: z.number() }))
      .query(async ({ input }) => {
        const dbInst = await db.getDb();
        if (!dbInst) return { ok: false };
        const { matchSettlements, tennisMatches } = await import("../drizzle/schema");
    ...
    debugSettlementList: adminProcedure
      .input(z.object({}))
      .query(async ({ input }) => {
        const dbInst = await db.getDb();
        if (!dbInst) return { ok: false };
        ...
        const rows = await dbInst.select({
          ...
          .leftJoin(tennisMatches, eq(matchSettlements.matchId, tennisMatches.id))
          .orderBy(matchSettlements.matchId)
          .limit(100);
        return {
          ok: true,
    ...
    debugRetrySettlement: adminProcedure
      .input(z.object({ matchId: z.number(), dryRun: z.boolean().optional() }))
      .mutation(async ({ input }) => {
        const dbInst = await db.getDb();
        if (!dbInst) return { ok: false, reason: "db unavailable" };
        const { matchSettlements, matchOrders, tennisMatches } = await import("../drizzle/schema");
        const { eq, and, inArray } = await import("drizzle-orm");
        const [settlement] = await dbInst.select().from(matchSettlements).where(eq(matchSettlements.matchId, input.matchId));
        if (!settlement) return { ok: false, reason: "settlement not found" };
        const organizer = await db.getUserById(settlement.organizerId);
        const payeeOpenid = organizer?.wechatOpenid || "";
        const netYuan = Number(settlement.netAmount || 0);
        const settleFen = Math.round(netYuan * 100);
        const [matchRecord] = await dbInst.select({ title: tennisMatches.title }).from(tennisMatches).where(eq(tennisMatches.id, input.matchId));
        const safeTitle = matchRecord?.title || "球局" + input.matchId;
        const shortTitle = (safeTitle || "").substring(0, 16);
        const summary = {
          matchId: input.matchId,
          status: settlement.status,
          wxBatchId: settlement.wxBatchId,
          hasPackage: !!decodeTransferPackageInfo(settlement.disputeReason),
          netAmount: netYuan,
          organizerId: settlement.organizerId,
          hasOpenid: !!payeeOpenid,
          transferSceneId: wxpayConfig.transferSceneId || "EMPTY",
          transferNotifyUrl: wxpayConfig.transferNotifyUrl || "EMPTY",
          transferSceneInfoType: wxpayConfig.transferSceneInfoType || "EMPTY",
        };
        if (input.dryRun) {
          return { ok: true, dryRun: true, nextAction: settlement.wxBatchId ? "query-existing" : "create-transfer", ...summary };
        }
        if (settlement.status === "disputed") return { ok: false, reason: "disputed", summary };
        if (!payeeOpenid) return { ok: false, reason: "no openid", summary };
        if (settleFen <= 0) return { ok: false, reason: "amount <= 0", summary };

        let transferResult: { status: string; packageInfo?: string; hasPackage: boolean } | undefined;
        let cancelState: string | null = null;
        let queryState: string | null = null;
        try {
          if (settlement.wxBatchId) {
            const existingBillNo = settlement.wxBatchId;
            try {
              const q = await queryTransferBill(existingBillNo);
              queryState = q.state;
              if (q.state === "SUCCESS") {
                await dbInst.update(matchSettlements).set({ status: "settled", settledAt: new Date() }).where(eq(matchSettlements.matchId, input.matchId));
                ...
                return { ok: true, action: "already-success", queryState, summary: { ...summary, status: "settled" } };
              }
              if (q.state === "ACCEPTED") {
                const resumed = await transferToUser({
                  batchId: existingBillNo,
                  openid: payeeOpenid,
                  amountFen: settleFen,
                  remark: buildMatchTransferRemark(shortTitle),
                });
                transferResult = { status: resumed.status, packageInfo: resumed.packageInfo, hasPackage: !!resumed.packageInfo };
                if (resumed.status === "SUCCESS") {
                  await dbInst.update(matchSettlements).set({ status: "settled", settledAt: new Date() }).where(eq(matchSettlements.matchId, input.matchId));
                  await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
                    .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
                } else if (resumed.packageInfo) {
                  await dbInst.update(matchSettlements).set({ status: "confirming", disputeReason: encodeTransferPackageInfo(resumed.packageInfo) })
                    .where(eq(matchSettlements.matchId, input.matchId));
                }
                return { ok: true, action: "resume-original", queryState, transfer: transferResult, summary: { ...summary, status: resumed.status === "SUCCESS" ? "settled" : "confirming" } };
              }
              if (q.state === "PROCESSING" || q.state === "CANCELING") {
                await dbInst.update(matchSettlements).set({ status: "confirming" }).where(eq(matchSettlements.matchId, input.matchId));
                return { ok: true, action: "keep-processing", queryState, transfer: { status: q.state }, summary: { ...summary, status: "confirming" } };
              }
              if (q.state === "WAIT_USER_CONFIRM" || q.state === "TRANSFERING") {
                const storedPkg = decodeTransferPackageInfo(settlement.disputeReason);
                if (storedPkg) {
                  await dbInst.update(matchSettlements).set({ status: "confirming" }).where(eq(matchSettlements.matchId, input.matchId));
                  return {
                    ok: true,
                    action: "package-available",
                    queryState,
                    transfer: { status: q.state, packageInfo: storedPkg },
                    summary: { ...summary, status: "confirming" },
                  };
                }
                const cancelRes = await cancelTransferBill(existingBillNo);
                cancelState = cancelRes.state;
                if (cancelRes.state !== "CANCELLED") {
                  return { ok: true, action: "wait-cancelled", queryState, cancelState, transfer: { status: cancelRes.state }, summary: { ...summary, status: "confirming" } };
                }
                await dbInst.update(matchSettlements).set({ status: "confirming", wxBatchId: null }).where(eq(matchSettlements.matchId, input.matchId));
              }
            } catch (queryErr: any) {
              const qmsg = String(queryErr?.message || queryErr || "");
              queryState = "QUERY_ERROR: " + qmsg;
              const legacyInvalidBill = !isValidOutBillNo(existingBillNo) && (qmsg.includes("PARAM_ERROR") || qmsg.includes("INVALID_REQUEST"));
              if (!(qmsg.includes("404") || qmsg.includes("ORDER_NOT_EXIST") || qmsg.includes("NOT_FOUND") || qmsg.includes("BILL_NOT_EXIST") || legacyInvalidBill)) {
                return { ok: false, reason: "query-error", queryState, summary };
              }
            }
          }

          const transfer = await transferToUser({ batchId: generateOrderId(), openid: payeeOpenid, amountFen: settleFen, remark: buildMatchTransferRemark(shortTitle) });
          transferResult = { status: transfer.status, packageInfo: transfer.packageInfo, hasPackage: !!transfer.packageInfo };
          if (transfer.status === "SUCCESS") {
            ...
            await dbInst.update(matchSettlements).set({
              ...
              ...(transfer.packageInfo ? { disputeReason: encodeTransferPackageInfo(transfer.packageInfo) } : {}),
            }).where(eq(matchSettlements.matchId, input.matchId));
          }
          const [after] = await dbInst.select().from(matchSettlements).where(eq(matchSettlements.matchId, input.matchId));
          return {
            ok: true,
            action: "transfer",
            queryState,
            ...
            summary: {
              ...summary,
              status: after?.status,
              wxBatchId: after?.wxBatchId,
              hasPackage: !!decodeTransferPackageInfo(after?.disputeReason),
            },
          };
        } catch (e: any) {
          return {
  ...
  coach: router({
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        // v1.5.97 暂不开放教练业务，避免未完成页面及教练资料进入公开发布面。
        if (!COACH_PUBLIC_FEATURE_ENABLED) return [];
    ...
    getById: publicProcedure
      ...
      .query(async ({ input }) => {
        if (!COACH_PUBLIC_FEATURE_ENABLED) {
          throw new TRPCError({ code: "NOT_FOUND", message: "教练功能暂未开放" });
        }
  ...
  match: router({
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        // 招募/满员列表默认从今天开始查，防止大量历史脏状态占满 limit，挤掉真正的未来球局。
        if ((queryInput.status === 'open' || queryInput.status === 'full') && !queryInput.dateFrom) {
          queryInput.dateFrom = chinaDateString();
        }
        const matches = await db.getTennisMatches(queryInput);
        // Haversine 距离计算（km）
        ...
        const enriched = await Promise.all(matches.map(async (m: any) => {
          ...
          return {
            ...m,
            organizerName: publicDisplayName(author?.name),
            organizerAvatar: author?.avatar ?? null,
            distanceKm,
          };
        }));
        // 过滤 circleOnly 球局（非圈内成员不可见）
        let publicMatches = enriched
          .filter((m: any) => !m.circleOnly)
          .map((m: any) => {
            // 定时任务未及时执行时，公开接口仍按真实时间展示为已完成，禁止过期球局继续伪装成招募中。
            if ((m.status === 'open' || m.status === 'full') && matchStartTimestamp(m) <= Date.now()) {
              return { ...m, status: 'completed' };
            }
            return m;
          });
        if (input?.status) {
          publicMatches = publicMatches.filter((m: any) => m.status === input.status);
        }
        // 附近模式：按距离过滤 + 排序
      ...
      .query(async ({ input, ctx }) => {
        ...
        const enrichedParticipants = await Promise.all(
          participants.map(async (p) => {
            ...
            return { ...p, user: user ? { id: user.id, name: publicDisplayName(user.name), avatar: user.avatar, ntrpLevel: user.ntrpLevel, creditScore: user.creditScore } : null };
          })
        );
        ...
        if (match.status === "open" || match.status === "full") {
          correctedStatus = matchStartTimestamp(match) <= Date.now()
            ? "completed"
            : (realCurrent >= maxP ? "full" : "open");
        }
        const correctedMatch = { ...match, currentParticipants: realCurrent, status: correctedStatus };
        // 详情页需要区分「结算处理中」和「已结算」，防止完成后继续显示确认入口。
        let settlementStatus: string | null = null;
        if (dbInstance && safeMatch.feeRequired && correctedStatus === "completed") {
          const { matchSettlements } = await import("../drizzle/schema");
          const { eq } = await import("drizzle-orm");
          const [settlement] = await dbInstance.select({ status: matchSettlements.status })
            .from(matchSettlements)
            .where(eq(matchSettlements.matchId, input.id))
            .limit(1);
        ...
        return {
          ...
          myParticipation,
          settlementStatus,
          author: author ? { id: author.id, name: publicDisplayName(author.name), avatar: author.avatar, ntrpLevel: author.ntrpLevel, creditScore: author.creditScore } : null,
        };
      }),
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const startAt = matchStartTimestamp(input);
        if (!Number.isFinite(startAt) || startAt <= Date.now()) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "球局开始时间必须晚于当前时间" });
        }
        if (input.endTime) {
          const endAt = new Date(`${input.matchDate}T${input.endTime}:00+08:00`).getTime();
        ...
        try {
          ...
          if (match) {
            ...
            if (organizer?.wechatOpenid) {
              ...
              const transfer = await transferToUser({ batchId, openid: organizer.wechatOpenid, amountFen: Math.round(Number(order.amount) * 100), remark: buildMatchTransferRemark(shortTopupTitle, "topup") });
              if (transfer.status === "SUCCESS") {
                ...
                const notice = transfer.status === "WAIT_USER_CONFIRM"
                  ? `「${match.title}」补缴 ${Number(order.amount).toFixed(2)} 元待您在微信确认收款。`
                  : transfer.status === "FAIL"
                    ? `「${match.title}」补缴 ${Number(order.amount).toFixed(2)} 元结算失败，请联系平台处理。`
                    : `「${match.title}」补缴 ${Number(order.amount).toFixed(2)} 元结算处理中，请稍后查看钱包。`;
                await sendNotification(match.authorId, "system", "补缴结算", notice, input.matchId);
              }
            }
          }
        } catch (e: any) {
    ...
    createPayOrder: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 同一用户同一球局只复用一个有效预付订单，禁止刷新页面后生成第二笔可支付订单。
        const previousOrders = await dbInst.select().from(matchOrders)
          .where(and(
            eq(matchOrders.matchId, input.matchId),
            eq(matchOrders.userId, ctx.user.id),
            eq(matchOrders.orderType, "prepay"),
            inArray(matchOrders.status, ["pending", "paid", "settled"]),
          ))
          .orderBy(desc(matchOrders.id));

        const paidOrder = previousOrders.find(order => order.status === "paid" || order.status === "settled");
        if (paidOrder) {
          if (participant.status === "pending") {
            if ((match.currentParticipants ?? 0) >= (match.maxParticipants ?? 0)) {
              throw new TRPCError({ code: "CONFLICT", message: "检测到已付款，但球局名额已满，请联系平台处理退款" });
            }
            const recovered = await dbInst.update(mp)
              .set({ status: "confirmed", paymentStatus: "paid", orderId: paidOrder.orderId })
              .where(and(eq(mp.matchId, input.matchId), eq(mp.userId, ctx.user.id), eq(mp.status, "pending")));
            if (getAffectedRows(recovered) > 0) {
              await dbInst.update(tm)
                .set({ currentParticipants: sql`${tm.currentParticipants} + 1` as any })
                .where(eq(tm.id, input.matchId));
              const [updatedMatch] = await dbInst.select().from(tm).where(eq(tm.id, input.matchId)).limit(1);
              if (updatedMatch && (updatedMatch.currentParticipants ?? 0) >= (updatedMatch.maxParticipants ?? 0) && updatedMatch.status === "open") {
                await dbInst.update(tm).set({ status: "full" }).where(eq(tm.id, input.matchId));
              }
            }
          } else {
            await dbInst.update(mp)
              .set({ paymentStatus: "paid", orderId: paidOrder.orderId })
              .where(and(eq(mp.matchId, input.matchId), eq(mp.userId, ctx.user.id)));
          }
          throw new TRPCError({ code: "CONFLICT", message: "支付已到账，报名已自动恢复，请刷新页面" });
        }

        const pendingOrder = previousOrders.find(order => order.status === "pending");
        if (pendingOrder) {
          if (!pendingOrder.wxPrepayId) {
            const createdAtMs = pendingOrder.createdAt ? new Date(pendingOrder.createdAt).getTime() : Date.now();
            if (Date.now() - createdAtMs < 2 * 60 * 1000) {
              throw new TRPCError({ code: "CONFLICT", message: "支付订单正在生成，请稍后重试" });
            }
            // 预下单进程中断后可能留下没有 prepayId 的僵尸订单，超过两分钟即可安全重建。
            await dbInst.delete(matchOrders)
              .where(and(eq(matchOrders.orderId, pendingOrder.orderId), eq(matchOrders.status, "pending")));
            await dbInst.update(mp)
              .set({ orderId: null })
              .where(and(
                eq(mp.matchId, input.matchId),
                eq(mp.userId, ctx.user.id),
                eq(mp.orderId, pendingOrder.orderId),
              ));
          } else {
            return {
              orderId: pendingOrder.orderId,
              ...buildClientPaymentParams(pendingOrder.wxPrepayId),
              isMockMode: false,
              reused: true,
            };
          }
        ...
        try {
          ...
          // 预下单失败不保留不可支付的 pending 订单，否则后续点击会一直被这笔订单阻塞。
          await dbInst.delete(matchOrders)
            .where(and(eq(matchOrders.orderId, orderId), eq(matchOrders.status, "pending")));
          await dbInst.update(mp)
            .set({ orderId: null })
            .where(and(eq(mp.matchId, input.matchId), eq(mp.userId, ctx.user.id), eq(mp.orderId, orderId)));
    ...
    confirmMatchComplete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.attendance && input.attendance.length > 0) {
          ...
          // 结算给发起人 = 实收总额 - 退款（无抽成, 发起人不分摊场地费）
          const settleFen = Math.round(totalPaid * 100) - refundTotalFen;
          ...
          // 打款给发起人（已收部分）
          let settled = false;
          let transferAction: { status: string; packageInfo?: string; mchId: string } | undefined;
          let settleMessage = `按实际到场 ${attendedCount} 人重算完成`;
          try {
            const organizer = await db.getUserById(ctx.user.id);
            const payeeOpenid = organizer?.wechatOpenid;
            console.log(`[settle] match=${input.matchId} totalPaid=${totalPaid} settleFen=${settleFen} payeeOpenid=${!!payeeOpenid} orders=${usedPaidOrders.length}`);
            if (payeeOpenid && settleFen > 0) {
              const shortTitle = (safeMatch.title || '').substring(0, 16);
              if (settleFen > 0) {
                const splitResult = await processSplitSettlementTransfer({
                  db: dbInst,
                  matchId: input.matchId,
                  organizerId: ctx.user.id,
                  openid: payeeOpenid,
                  totalFen: settleFen,
                  matchTitle: shortTitle,
                });
                transferAction = splitResult.transfer;
                settled = splitResult.settled;
                settleMessage = splitResult.message;
                return { success: true, mode: "dynamic", attendedCount, divisor, organizerAttended, organizerShare: organizerShareFen / 100, perPerson: perFen / 100, refundTotal: refundTotalFen / 100, topupTotal: topupTotalFen / 100, settled, transfer: transferAction, progress: splitResult.progress, message: settleMessage };
              }
              const batchId = generateOrderId();
              const transfer = await transferToUser({ batchId, openid: payeeOpenid, amountFen: settleFen, remark: buildMatchTransferRemark(shortTitle) });
              transferAction = { status: transfer.status, packageInfo: transfer.packageInfo, mchId: wxpayConfig.mchId };
              if (transfer.status === "SUCCESS") {
                await dbInst.update(matchSettlements).set({ status: "settled", settledAt: new Date(), wxBatchId: transfer.batchId }).where(eq(matchSettlements.matchId, input.matchId));
                await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
                  .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
                settled = true;
                settleMessage = `已结算 ${settleYuan.toFixed(2)} 元到您的微信零钱`;
              } else {
                await dbInst.update(matchSettlements).set({
                  status: "confirming",
                  wxBatchId: transfer.batchId,
                ...
                settleMessage = transfer.status === "WAIT_USER_CONFIRM" ? "请在微信确认收款后到账" : "转账处理中，请稍后查看钱包";
              }
            } else if (!payeeOpenid) {
          ...
          return { success: true, mode: "dynamic", attendedCount, divisor, organizerAttended, organizerShare: organizerShareFen / 100, perPerson: perFen / 100, refundTotal: refundTotalFen / 100, topupTotal: topupTotalFen / 100, settled, transfer: transferAction, message: settleMessage };
        }
        ...
        try {
          ...
          if (!payeeOpenid) {
            ...
            if (amountFen > 0) {
              ...
              const batchId = generateOrderId();
            const transfer = await transferToUser({ batchId, openid: payeeOpenid, amountFen, remark: buildMatchTransferRemark(shortTitle) });
            transferAction = { status: transfer.status, packageInfo: transfer.packageInfo, mchId: wxpayConfig.mchId };
            if (transfer.status === "SUCCESS") {
              await dbInst.update(matchSettlements).set({ status: "settled", settledAt: new Date(), wxBatchId: transfer.batchId }).where(eq(matchSettlements.matchId, input.matchId));
              await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
        ...
        return { success: true, totalAmount, settled, transfer: transferAction, message: settleMessage };
      }),

    // ─── 重新发起结算打款（已结算但未打款成功时可重试）────────────────────────
    retrySettlement: protectedProcedure
      .input(z.object({ matchId: z.number() }))
      .mutation(async ({ ctx, input }) => {
        const dbInst = await db.getDb();
        if (!dbInst) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR' });
        ...
        const { eq, and, inArray } = await import("drizzle-orm");
        const [settlement] = await dbInst.select().from(matchSettlements).where(eq(matchSettlements.matchId, input.matchId));
        if (!settlement) throw new TRPCError({ code: 'NOT_FOUND', message: '未找到结算记录' });
        // 兼容历史误标：status=settled 也允许进入查单流程，只有微信查询为 SUCCESS 才保持已结算
        if (settlement.status === 'disputed') throw new TRPCError({ code: 'BAD_REQUEST', message: '结算有异议，请联系平台' });
        if (settlement.organizerId !== ctx.user.id) throw new TRPCError({ code: 'FORBIDDEN', message: '只有发起人可操作' });
        const organizer = await db.getUserById(ctx.user.id);
        const payeeOpenid = organizer?.wechatOpenid;
        const netYuan = Number(settlement.netAmount || 0);
        const settleFen = Math.round(netYuan * 100);
        if (!payeeOpenid) throw new TRPCError({ code: 'BAD_REQUEST', message: '未获取到您的微信收款信息' });
        if (settleFen <= 0) throw new TRPCError({ code: 'BAD_REQUEST', message: '结算金额为0，无需打款' });
        const [matchRecord] = await dbInst.select({ title: tennisMatches.title }).from(tennisMatches).where(eq(tennisMatches.id, input.matchId));
        const safeTitle = matchRecord?.title || '球局' + input.matchId;
        const shortTitle = (safeTitle || "").substring(0, 16);
        if (settleFen > MAX_SINGLE_TRANSFER_FEN || await hasSplitSettlementPlan(dbInst, input.matchId)) {
          try {
            return await processSplitSettlementTransfer({
              db: dbInst,
              matchId: input.matchId,
              organizerId: settlement.organizerId,
              openid: payeeOpenid,
            ...
            console.error(`[retrySettlement:split] FAILED match=${input.matchId}:`, e?.message || e);
            throw new TRPCError({ code: 'BAD_REQUEST', message: '分笔打款失败: ' + (e?.message || '未知错误') });
          }
        }
        try {
          if (settlement.wxBatchId) {
            ...
            try {
              const q = await queryTransferBill(existingBillNo);
              console.log(`[retrySettlement] query match=${input.matchId} bill=${existingBillNo} state=${q.state}`);
              if (q.state === "SUCCESS") {
                await dbInst.update(matchSettlements).set({ status: 'settled', settledAt: new Date() }).where(eq(matchSettlements.matchId, input.matchId));
                await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
                  .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
                return { success: true, settled: true, transfer: { status: q.state, mchId: wxpayConfig.mchId }, message: `已结算 ${netYuan.toFixed(2)} 元到您的微信零钱` };
              }
              if (q.state === "ACCEPTED") {
                // 微信文档明确：ACCEPTED 为“已受理，可原单重试”。必须复用原 out_bill_no，
                // 不能换新单号，否则既无法推进到 WAIT_USER_CONFIRM，也有重复转账风险。
                const resumed = await transferToUser({
                  batchId: existingBillNo,
                  openid: payeeOpenid,
                  amountFen: settleFen,
                ...
                if (resumed.status === "SUCCESS") {
                  ...
                  await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
                    .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
                  return { success: true, settled: true, transfer: { status: resumed.status, mchId: wxpayConfig.mchId }, message: `已结算 ${netYuan.toFixed(2)} 元到您的微信零钱` };
                }
                await dbInst.update(matchSettlements).set({
                  status: 'confirming',
                  ...(resumed.packageInfo ? { disputeReason: encodeTransferPackageInfo(resumed.packageInfo) } : {}),
                }).where(eq(matchSettlements.matchId, input.matchId));
                return {
                  success: true,
                  settled: false,
                  transfer: { status: resumed.status, packageInfo: resumed.packageInfo, mchId: wxpayConfig.mchId },
                  message: resumed.status === "WAIT_USER_CONFIRM" ? "请确认收款后到账" : "转账处理中，请稍后查看钱包",
                };
              }
              if (q.state === "PROCESSING" || q.state === "CANCELING") {
                await dbInst.update(matchSettlements).set({ status: 'confirming' }).where(eq(matchSettlements.matchId, input.matchId));
                return {
                  success: true,
                  settled: false,
                  transfer: { status: q.state, mchId: wxpayConfig.mchId },
                  message: q.state === "CANCELING" ? "原转账正在安全撤销，完成后请再点一次继续收款" : "转账处理中，请稍后查看钱包",
                };
              }
              if (q.state === "WAIT_USER_CONFIRM" || q.state === "TRANSFERING") {
                const storedPkg = decodeTransferPackageInfo(settlement.disputeReason);
                if (storedPkg) {
                  ...
                  return {
                    success: true,
                    settled: false,
                    transfer: { status: q.state, packageInfo: storedPkg, mchId: wxpayConfig.mchId },
                    message: "请在微信确认收款后到账",
                  };
                }
                // 历史单据没有暂存 package_info，无法再次拉起确认页；先撤销旧单，再重新发起转账。
                // package_info 无法通过查单重新获取。先撤销旧单，但必须等到 CANCELLED 终态后
                // 才能创建新单，避免“旧单仍可确认 + 新单已创建”的双付窗口。
                const cancelRes = await cancelTransferBill(existingBillNo);
                console.log(`[retrySettlement] cancel old bill match=${input.matchId} bill=${existingBillNo} state=${cancelRes.state}`);
                if (cancelRes.state !== "CANCELLED") {
                  await dbInst.update(matchSettlements).set({ status: 'confirming' }).where(eq(matchSettlements.matchId, input.matchId));
                  return {
                    success: true,
                    settled: false,
                    transfer: { status: cancelRes.state, mchId: wxpayConfig.mchId },
                    message: "原转账正在安全撤销，完成后请再点一次继续收款",
                  };
                }
                await dbInst.update(matchSettlements).set({ status: 'confirming', wxBatchId: null }).where(eq(matchSettlements.matchId, input.matchId));
                console.log(`[retrySettlement] old bill cancelled; create replacement match=${input.matchId}`);
              }
            } catch (queryErr: any) {
              const qmsg = String(queryErr?.message || queryErr || "");
              const legacyInvalidBill = !isValidOutBillNo(existingBillNo) && (qmsg.includes("PARAM_ERROR") || qmsg.includes("INVALID_REQUEST"));
              if (!(qmsg.includes("404") || qmsg.includes("ORDER_NOT_EXIST") || qmsg.includes("NOT_FOUND") || qmsg.includes("BILL_NOT_EXIST") || legacyInvalidBill)) {
                throw new TRPCError({ code: "BAD_REQUEST", message: "无法确认原转账状态，请稍后重试" });
              }
              console.warn(`[retrySettlement] query not found match=${input.matchId} bill=${existingBillNo}:`, qmsg);
            }
          }

          console.log(`[retrySettlement] match=${input.matchId} amountFen=${settleFen}`);
          const transfer = await transferToUser({ batchId: generateOrderId(), openid: payeeOpenid, amountFen: settleFen, remark: buildMatchTransferRemark(shortTitle) });
          if (transfer.status === "SUCCESS") {
            await dbInst.update(matchSettlements).set({ status: 'settled', settledAt: new Date(), wxBatchId: transfer.batchId }).where(eq(matchSettlements.matchId, input.matchId));
            await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
              .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
            console.log(`[retrySettlement] SUCCESS match=${input.matchId}`);
            return { success: true, settled: true, transfer: { status: transfer.status, mchId: wxpayConfig.mchId }, message: `已结算 ${netYuan.toFixed(2)} 元到您的微信零钱` };
          }
          await dbInst.update(matchSettlements).set({
            status: 'confirming',
            wxBatchId: transfer.batchId,
          ...
          console.log(`[retrySettlement] PENDING match=${input.matchId} state=${transfer.status}`);
          return {
            success: true,
            settled: false,
            transfer: { status: transfer.status, packageInfo: transfer.packageInfo, mchId: wxpayConfig.mchId },
            message: transfer.status === "WAIT_USER_CONFIRM" ? "请在微信确认收款后到账" : "转账处理中，请稍后查看钱包",
          };
        } catch (e: any) {
          console.error(`[retrySettlement] FAILED match=${input.matchId}:`, e?.message || e);
          throw new TRPCError({ code: 'BAD_REQUEST', message: '打款失败: ' + (e?.message || '未知错误') });
        }
      }),
    ...
    myWallet: protectedProcedure
      .query(async ({ ctx }) => {
        ...
        // 回调可能因网络/验签等原因延迟或丢失。钱包打开时主动查一次微信终态，
        // 只在微信明确返回 SUCCESS 后落库，既能自动修复“实际已到账但仍显示处理中”，
        // 也不会把 WAIT_USER_CONFIRM/PROCESSING 误标为已结算。
        for (const settlement of mySettlements as any[]) {
          const totalFen = Math.round(Number(settlement.netAmount || 0) * 100);
          try {
            const progress = await syncSplitSettlementTransfer({ db: dbInst, matchId: settlement.matchId, totalFen });
            if (progress) {
              settlement.transferProgress = progress;
              if (progress.completedSequences === progress.totalSequences) {
            ...
            console.warn(`[myWallet] sync split settlement failed match=${settlement.matchId}:`, error?.message || error);
          }
          if (settlement.status !== "confirming" || !settlement.wxBatchId) continue;
          try {
            const transfer = await queryTransferBill(settlement.wxBatchId);
            if (transfer.state !== "SUCCESS") continue;
            const settledAt = new Date();
            await dbInst.update(matchSettlements).set({ status: "settled", settledAt })
              .where(eq(matchSettlements.matchId, settlement.matchId));
            await dbInst.update(matchOrders).set({ status: "settled", settledAt })
              .where(and(eq(matchOrders.matchId, settlement.matchId), inArray(matchOrders.status, ["paid"])));
            settlement.status = "settled";
            settlement.settledAt = settledAt;
          } catch (error: any) {
            console.warn(`[myWallet] sync settlement failed match=${settlement.matchId}:`, error?.message || error);
          }
        }
        ...
        return {
          ...
          pendingSettlements: mySettlements.filter(s => s.status === "confirming" || (s.status === "settled" && !s.wxBatchId)),
          settledSettlements: mySettlements.filter(s => s.status === "settled" && !!s.wxBatchId),
