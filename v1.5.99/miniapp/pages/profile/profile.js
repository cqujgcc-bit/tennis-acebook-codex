...
Page({
  data: {
    user: { nickname: '未登录', level: '', city: '', bio: '', gamesCount: 0, circlesCount: 0, creditScore: '--' },
    isLogin: false,
    unreadCount: 0,
    isAdmin: false,
  },
  ...
  onShow() {
    ...
    if (!token) {
      this.setData({
        ...
        isAdmin: false,
        unreadCount: 0,
        user: { nickname: '未登录', level: '', city: '点击登录', bio: '', gamesCount: 0, circlesCount: 0, creditScore: '--' },
      });
      try { wx.removeTabBarBadge({ index: 3 }); } catch (e) {}
      return;
    }
  ...
  loadUnread() {
    ...
    api.notification
      ...
      .then(function (r) {
        ...
        try {
          if (n > 0) {
            wx.setTabBarBadge({ index: 3, text: String(n > 99 ? '99+' : n) });
          } else {
  ...
  loadProfile() {
    ...
    this.setData({
      user: {
        ...
        gamesCount: 0,
        circlesCount: 0,
        creditScore: stored.creditScore == null ? '--' : stored.creditScore,
      },
    });
    // 拉取真实统计
    api.user
