...
Page({
  ...
  decorate(list) {
    ...
    return (list || []).map(function (it) {
      ...
      return Object.assign({}, it, {
        ...
        progressText: it.transferProgress
          ? ('已到账 ' + it.transferProgress.completedSequences + '/' + it.transferProgress.totalSequences
            + ' 笔（¥' + that.fmtAmount(it.transferProgress.completedAmountFen / 100) + '）')
          : '',
      });
    });
  },
  ...
  onRetrySettlement(e) {
    ...
    wx.showModal({
      title: '继续收款',
      content: '系统会先核对原转账单。超过200元将自动拆分，每笔不超过200元，并且上一笔确认到账后才会发起下一笔，不会重复打款。',
      confirmText: '继续收款',
      success: function (r) {
        if (!r.confirm) return;
        ...
        api.match.retrySettlement(matchId).then(function (res) {
          wx.hideLoading();
          var transfer = res && res.transfer;
          if (transfer && transfer.status === 'WAIT_USER_CONFIRM' && transfer.packageInfo) {
            if (!wx.canIUse('requestMerchantTransfer')) {
              wx.showModal({ title: '收款确认', content: '当前微信版本不支持确认收款，请更新微信后重试。', showCancel: false });
              that.loadWallet();
              return;
            }
            wx.requestMerchantTransfer({
              ...
              package: transfer.packageInfo,
              success: function () {
                // JSAPI success 只表示成功打开收款页，不代表资金已经到账；
                // 重新加载钱包会触发后端主动查单，以微信 SUCCESS 终态为准。
                wx.showToast({ title: '正在核对本笔到账结果', icon: 'none', duration: 3000 });
                setTimeout(function () { that.loadWallet(); }, 1200);
              },
              fail: function (err) {
