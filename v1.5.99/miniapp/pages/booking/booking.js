...
function decorate(list, now) {
  ...
  return (list || []).map(function (m) {
    ...
    return {
      ...
      isInactive: status === 'completed' || status === 'expired' || status === 'cancelled',
      _raw: m,
    };
  });
}
...
Page({
  ...
  loadMatches: function (fromPull, extraInput) {
    ...
    var input = Object.assign({ limit: 50, dateFrom: localDateString(new Date()), includeCancelled: true }, extraInput || {});
    api.match
      .list(input)
      .then(function (list) {
        var decorated = decorate(list, new Date());
        that.setData({ allGames: decorated, loading: false, historyLoaded: false });
        if (that.data.activeFilter === 'history') that.loadHistory();
        else that.applyFilter(that.data.activeFilter);
  ...
  onFilterTap: function (e) {
    ...
    if (key === 'history' && !this.data.historyLoaded) {
      this.loadHistory();
      return;
    }
  ...
  loadHistory: function () {
    var that = this;
    this.setData({ loading: true, activeFilter: 'history' });
    api.match
      .list({ limit: 50, dateTo: previousDateString(), includeCancelled: true })
      .then(function (list) {
        var combined = (that.data.allGames || []).concat(decorate(list, new Date()));
        var seen = {};
        ...
        that.setData({ allGames: combined, historyLoaded: true, loading: false });
        that.applyFilter('history');
      })
      .catch(function (err) {
        that.setData({ loading: false });
        wx.showToast({ title: (err && err.message) || '历史球局加载失败', icon: 'none' });
      });
  },
  ...
  enableNearby: function () {
    ...
    wx.getLocation({
      ...
      success: function (res) {
        ...
        api.match
          .list({ limit: 50, dateFrom: localDateString(new Date()), nearbyLat: res.latitude, nearbyLng: res.longitude })
          .then(function (list) {
            var decorated = decorate(list, new Date());
            // 按距离升序（无距离的排后面）
