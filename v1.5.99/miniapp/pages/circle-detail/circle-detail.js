...
Page({
  ...
  loadMatches() {
    ...
    api.circle
      ...
      .then(function (list) {
        ...
        history.sort(function (a, b) { return b.sortKey.localeCompare(a.sortKey); });
        var c = that.data.circle || {};
        c.activeGames = active.length;
        that.setData({ matchesActive: active, matchesHistory: history, circle: c });
      })
      .catch(function () {
        that.setData({ matchesActive: [], matchesHistory: [] });
      });
  },
  ...
  loadActivities() {
    ...
    api.circle.getActivities(this.circleId).then(function (list) {
      ...
      (list || []).forEach(function (a) {
        ...
        var isHistory = isPast && (a.settleStatus === 'settled' || a.settleStatus === 'settling');
        var item = {
          id: a.id,
          title: a.title || '活动',
          dateStr: dateStr,
          ...
          sortKey: (a.activityDate || '') + ' ' + (a.startTime || ''),
        };
        if (isHistory) {
          history.push(item);
        } else {
      ...
      active.sort(function (a, b) { return a.sortKey.localeCompare(b.sortKey); });
      history.sort(function (a, b) { return b.sortKey.localeCompare(a.sortKey); });
      that.setData({ activitiesActive: active, activitiesHistory: history });
    }).catch(function () {
      that.setData({ activitiesActive: [], activitiesHistory: [] });
    });
  },
  ...
  // ── 进入圈子活动列表 ──
  // ── 圈内球局历史折叠 ──
  toggleMatchHistory: function () {
    this.setData({ matchHistoryShow: !this.data.matchHistoryShow });
  },
  // ── 圈内活动历史折叠 ──
  toggleActivityHistory: function () {
    this.setData({ activityHistoryShow: !this.data.activityHistoryShow });
  },
