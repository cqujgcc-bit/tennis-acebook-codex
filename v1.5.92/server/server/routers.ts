...
// 临时诊断密钥，仅用于 v1.5.88 定位线上结算问题，下一版本会移除
const WX_DEBUG_KEY = "acebook-debug-20260801";
...
import { createPrepay, refundOrder, transferToUser, queryTransferBill, cancelTransferBill, encodeTransferPackageInfo, decodeTransferPackageInfo, generateOrderId, isWxpayConfigured, wxpayConfig, isValidOutBillNo } from "./wxpay";
...
export const appRouter = router({
  ...
  auth: router({
    ...
    debugSettlement: publicProcedure
      .input(z.object({ matchId: z.number(), key: z.string() }))
      .query(async ({ input }) => {
        if (input.key !== WX_DEBUG_KEY) {
    ...
    debugSettlementList: publicProcedure
    ...
    debugRetrySettlement: publicProcedure
      .input(z.object({ matchId: z.number(), key: z.string(), dryRun: z.boolean().optional() }))
      .mutation(async ({ input }) => {
        if (input.key !== WX_DEBUG_KEY) {
          throw new TRPCError({ code: "UNAUTHORIZED", message: "invalid key" });
        }
