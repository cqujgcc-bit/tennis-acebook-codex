...
var users, coachProfiles, venues, coachVenues, coachAvailability, bookings, payments, reviews, notifications, coupons, couponUsages, smsCodes, inviteUsages, coachLocations, tennisMatches, matchParticipants, lessonPackages, studentPackages, packageDeductions, matchReviews, circles, circleMembers, circlePosts, circlePostLikes, circleCheckins, circleActivities, circleActivitySignups, partnerVenues, venueAvailableSlots, creditLogs, matchReplaceInvites, matchOrders, matchSettlements, matchMessages, feedbacks;
var init_schema = __esm({
  "drizzle/schema.ts"() {
    ...
    matchParticipants = mysqlTable("match_participants", {
      ...
      userId: int("userId").notNull(),
      status: mysqlEnum("status", ["pending", "confirmed", "rejected", "cancelled"]).default("confirmed").notNull(),
...
__export(wechat_exports, {
  ...
  getPhoneNumberByCode: () => getPhoneNumberByCode,
...
async function getPhoneNumberByCode(code) {
  ...
  if (data.errcode !== 0 || !data.phone_info) {
    throw new Error(`[WeChat] getPhoneNumber failed: ${data.errmsg} (${data.errcode})`);
  }
...
var appRouter = router({
  ...
  auth: router({
    ...
    loginWithWechat: publicProcedure.input(z2.object({
      ...
      if (phoneCode && !user.phone) {
        try {
          const { getPhoneNumberByCode: getPhoneNumberByCode2 } = await Promise.resolve().then(() => (init_wechat(), wechat_exports));
          const phone = await getPhoneNumberByCode2(phoneCode);
          if (phone) {
  ...
  match: router({
    ...
    getById: publicProcedure.input(z2.object({ id: z2.number() })).query(async ({ input, ctx }) => {
      ...
      return { match, participants: enrichedParticipants, author: author ? { id: author.id, name: author.name, avatar: author.avatar, ntrpLevel: author.ntrpLevel, creditScore: author.creditScore } : null };
    }),
    create: protectedProcedure.input(z2.object({
      ...
      const newMatch = await createTennisMatch({ authorId: ctx.user.id, ...input, imageUrl: resolvedImageUrl });
    ...
    join: protectedProcedure.input(z2.object({ matchId: z2.number(), message: z2.string().optional() })).mutation(async ({ ctx, input }) => {
      ...
      if (existing) {
        ...
        if (dbInst) {
          ...
          if (needsPayment) updateData.paymentStatus = "pending";
          await dbInst.update(mp).set(updateData).where(and2(eq2(mp.matchId, input.matchId), eq2(mp.userId, ctx.user.id)));
        }
      } else {
        await joinTennisMatch(input.matchId, ctx.user.id, input.message, needsPayment ? "pending" : "not_required");
      }
    ...
    leave: protectedProcedure.input(z2.object({ matchId: z2.number() })).mutation(async ({ ctx, input }) => {
      ...
      await updateTennisMatchParticipantCount(input.matchId, -1);
      if (match.status === "full") await updateTennisMatchStatus(input.matchId, "open");
      if (isPastDeadline) {
        await addCreditLog(
          ctx.user.id,
          -10,
      ...
      return { success: true, penalized: false, deducted: 0, refunded, refundMessage };
    }),
    ...
    acceptReplaceInvite: protectedProcedure.input(z2.object({ token: z2.string() })).mutation(async ({ ctx, input }) => {
      ...
      return { success: true, matchId: invite.matchId, needsPay };
    }),
    ...
    update: protectedProcedure.input(z2.object({
      ...
      if (fields.maxParticipants !== void 0) {
        if (fields.maxParticipants < match.currentParticipants) {
          throw new TRPCError3({ code: "BAD_REQUEST", message: `\u4EBA\u6570\u4E0A\u9650\u4E0D\u80FD\u4F4E\u4E8E\u5DF2\u62A5\u540D\u4EBA\u6570 (${match.currentParticipants})` });
        }
    ...
    confirmPayment: protectedProcedure.input(z2.object({ orderId: z2.string(), matchId: z2.number() })).mutation(async ({ ctx, input }) => {
      ...
      await dbInst.update(mp).set({ paymentStatus: "paid", orderId: input.orderId }).where(and2(eq2(mp.matchId, input.matchId), eq2(mp.userId, ctx.user.id)));
      return { success: true };
    }),
    // ─── 支付：发起者确认球局完成，触发结算流程 ──────────────────────────────
      ...
      for (const p of participants) {
        if (p.paymentStatus === "paid") {
          await sendNotification(
            ...
            "system",
            "\u7403\u5C40\u5DF2\u5B8C\u6210\uFF0C\u7ED3\u7B97\u4E2D",
            `\u300C${match.title}\u300D\u5DF2\u786E\u8BA4\u5B8C\u6210\uFF0C\u573A\u5730\u8D39\u5C06\u572824\u5C0F\u65F6\u540E\u7ED3\u7B97\u7ED9\u7EC4\u7EC7\u8005\u3002\u5982\u6709\u5F02\u8BAE\u8BF7\u572824\u5C0F\u65F6\u5185\u7533\u8BF7\u3002`,
            input.matchId
          );
        }
      }
      return { success: true, totalAmount, message: `\u5171 ${totalAmount} \u5143\u5C06\u572824\u5C0F\u65F6\u540E\u7ED3\u7B97` };
    }),
    // ─── 支付：参与者申请异议（球局未举行等情况）────────────────────────────
...
\u{1F3E0} \u5730\u5740\uFF1A${input.customVenueAddress}` : ""}`;
  ...
  circle: router({
    ...
    getPosts: protectedProcedure.input(z2.object({ circleId: z2.number() })).query(async ({ ctx, input }) => {
      const dbInstance = await getDb();
      if (!dbInstance) throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR" });
      const { circleMembers: circleMembers2, circlePosts: circlePosts2, users: users2 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
    ...
    getMatches: protectedProcedure.input(z2.object({ circleId: z2.number() })).query(async ({ ctx, input }) => {
      ...
      try {
        matches = await dbInstance.select({
          ...
          authorName: users2.name,
          authorAvatar: users2.avatar
        }).from(tennisMatches2).leftJoin(users2, eq2(tennisMatches2.authorId, users2.id)).where(and2(eq2(tennisMatches2.circleId, input.circleId))).orderBy(desc2(tennisMatches2.createdAt)).limit(30);
      } catch {
        matches = [];
      }
      return matches;
...
\u201C${input.content}\u201D` : "";
    ...
    createActivity: protectedProcedure.input(z2.object({
      ...
      repeatWeeks: z2.number().min(0).max(4).default(0)
      // 0=不重复, 1-4=重复周数
    })).mutation(async ({ ctx, input }) => {
      const dbInstance = await getDb();
      if (!dbInstance) throw new TRPCError3({ code: "INTERNAL_SERVER_ERROR" });
  ...
  app.post("/api/wxpay/notify", async (req, res) => {
    try {
      ...
      if (payResult.trade_state === "SUCCESS") {
        ...
        const db = await getDb2();
        if (db) {
          const { matchOrders: matchOrders2, matchParticipants: matchParticipants2 } = await Promise.resolve().then(() => (init_schema(), schema_exports));
          const { eq: eq2, and: and2 } = await import("drizzle-orm");
          ...
          if (orders.length > 0) {
            ...
            await db.update(matchParticipants2).set({ paymentStatus: "paid", orderId: order.orderId }).where(and2(eq2(matchParticipants2.matchId, order.matchId), eq2(matchParticipants2.userId, order.userId)));
          }
        }
      }
