...
Page({
  ...
  // 从结算页返回时必须重新拉取球局状态；首次 onShow 不重复请求。
  onShow() {
    if (this._hasShown) {
      this.loadDetail();
    }
  ...
  loadDetail() {
    ...
    api.match
      ...
      .then(function (res) {
        ...
        var settlementStatus = res.settlementStatus || '';
        ...
        var game = {
          ...
          completedLabel: !feeRequired ? '已结束'
            : (settlementStatus === 'settled' ? '已结算'
              : (settlementStatus === 'confirming' ? '结算处理中'
                : (settlementStatus === 'disputed' ? '结算有异议' : '已结束'))),
