...
import { createPrepay, refundOrder, transferToUser, generateOrderId, isWxpayConfigured, wxpayConfig } from "./wxpay";
  ...
  match: router({
          ...
          try {
            ...
            if (payeeOpenid && settleFen > 0) {
              ...
              if (transfer.status === "SUCCESS") {
                ...
                await dbInst.update(matchSettlements).set({ status: "confirming", wxBatchId: transfer.batchId }).where(eq(matchSettlements.matchId, input.matchId));
    ...
    retrySettlement: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (settlement.status === 'settled') throw new TRPCError({ code: 'BAD_REQUEST', message: '已结算' });
        ...
        try {
          ...
          const transfer = await transferToUser({ batchId, openid: payeeOpenid, amountFen: settleFen, remark: `球局「${shortTitle}」结算-重试` });
          await dbInst.update(matchSettlements).set({ status: 'settled', settledAt: new Date(), wxBatchId: transfer.batchId }).where(eq(matchSettlements.matchId, input.matchId));
          console.log(`[retrySettlement] SUCCESS match=${input.matchId}`);
          return { success: true, message: `已结算 ${netYuan.toFixed(2)} 元到您的微信零钱` };
        } catch (e: any) {
