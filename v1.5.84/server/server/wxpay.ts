...
export async function transferToUser(params: {
  ...
  if (!wxpayConfig.transferNotifyUrl || !wxpayConfig.transferSceneId || !wxpayConfig.transferSceneInfoType) {
    throw new Error("商家转账配置不完整：请配置 WXPAY_TRANSFER_NOTIFY_URL、WXPAY_TRANSFER_SCENE_ID、WXPAY_TRANSFER_SCENE_INFO_TYPE");
  }
  ...
  const resp = await wxpayRequest<{
    ...
    "/v3/fund-app/mch-transfer/transfer-bills",
    {
      ...
      user_recv_perception: params.remark,
      transfer_scene_id: wxpayConfig.transferSceneId,
      transfer_scene_report_infos: [{
        info_type: wxpayConfig.transferSceneInfoType,
        info_content: params.remark.slice(0, 32),
      }],
    }
  );
