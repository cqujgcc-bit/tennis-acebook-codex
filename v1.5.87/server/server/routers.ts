...
import { createPrepay, refundOrder, transferToUser, queryTransferBill, encodeTransferPackageInfo, decodeTransferPackageInfo, generateOrderId, isWxpayConfigured, wxpayConfig } from "./wxpay";
            ...
            try {
              ...
              if (q.state === "WAIT_USER_CONFIRM" || q.state === "TRANSFERING") {
                ...
                return {
                  success: true,
                  settled: false,
                  transfer: { status: q.state, packageInfo: decodeTransferPackageInfo(settlement.disputeReason), mchId: wxpayConfig.mchId },
                  message: "请在微信确认收款后到账",
                };
              }
            } catch (queryErr: any) {
              const qmsg = String(queryErr?.message || queryErr || "");
              if (!(qmsg.includes("404") || qmsg.includes("ORDER_NOT_EXIST") || qmsg.includes("NOT_FOUND") || qmsg.includes("BILL_NOT_EXIST"))) {
