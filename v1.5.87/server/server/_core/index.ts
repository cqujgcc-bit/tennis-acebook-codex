  ...
  app.post("/api/wxpay/transfer-notify", async (req, res) => {
    try {
      ...
      if (isWxpayConfigured()) {
        ...
        const valid = key && verifyNotifySignature(String(req.headers["wechatpay-timestamp"] || ""), String(req.headers["wechatpay-nonce"] || ""), rawBody, String(req.headers["wechatpay-signature"] || ""), key);
