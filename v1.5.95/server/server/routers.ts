...
import { buildClientPaymentParams, createPrepay, refundOrder, transferToUser, queryTransferBill, cancelTransferBill, encodeTransferPackageInfo, decodeTransferPackageInfo, generateOrderId, isWxpayConfigured, wxpayConfig, isValidOutBillNo } from "./wxpay";
    ...
    debugRetrySettlement: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          const transfer = await transferToUser({ batchId: generateOrderId(), openid: payeeOpenid, amountFen: settleFen, remark: `球局「${shortTitle}」结算-重试` });
        ...
        try {
          ...
          if (match) {
            ...
            if (organizer?.wechatOpenid) {
              ...
              const transfer = await transferToUser({ batchId, openid: organizer.wechatOpenid, amountFen: Math.round(Number(order.amount) * 100), remark: `球局「${shortTopupTitle}」补缴` });
              if (transfer.status === "SUCCESS") {
