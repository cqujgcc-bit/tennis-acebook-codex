    ...
    try {
      ...
      for (const settlement of pendingSettlements) {
        try {
          ...
          const { queryTransferBill, transferToUser, generateOrderId, encodeTransferPackageInfo } = await import("../wxpay");
          ...
          if (settled) {
            ...
            const transfer = await transferToUser({
              ...
              amountFen,
              remark: `球局「${settlement.matchId}」结算-自动`,
            });
              ...
              try {
                ...
                if (matchRowsT.length > 0) {
                  ...
                  if (organizer?.wechatOpenid) {
                    const { transferToUser } = await import("../wxpay");
                    // 使用订单号派生固定转账单号，即使进程中断也不会生成第二笔转账。
                    const batchId = `TU${order.orderId}`.replace(/[^A-Za-z0-9_-]/g, "").slice(0, 32);
                    const transfer = await transferToUser({ batchId, openid: organizer.wechatOpenid, amountFen: Math.round(Number(order.amount) * 100), remark: `球局「${mt.title}」补缴结算` });
