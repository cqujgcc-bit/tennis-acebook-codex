  ...
  match: router({
    ...
    confirmMatchComplete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.attendance && input.attendance.length > 0) {
          ...
          try {
            console.log(`[settle] match=${input.matchId} totalPaid=${totalPaid} settleFen=${settleFen} payeeOpenid=${!!organizer?.wechatOpenid} orders=${usedPaidOrders.length}`);
            ...
            if (payeeOpenid && settleFen > 0) {
              ...
              await dbInst.update(matchOrders).set({ status: "settled", settledAt: new Date() })
                .where(and(eq(matchOrders.matchId, input.matchId), inArray(matchOrders.status, ["paid"])));
              settled = true;
              settleMessage = `已结算 ${settleYuan.toFixed(2)} 元到您的微信零钱` + (topupTotalFen > 0 ? `；另有 ${(topupTotalFen/100).toFixed(2)} 元待到场者补缴` : "");
            } else if (!payeeOpenid) {
    ...
    retrySettlement: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const batchId = (await import("./wxpay")).generateOrderId();
        try {
          ...
          throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: '打款失败：' + (e?.message || '未知错误') });
        }
      }),
