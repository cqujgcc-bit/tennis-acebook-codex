...
// ─── Platform Transactions（平台资金流水，管理员可见不可篡改）───────────
export const platformTransactions = mysqlTable("platform_transactions", {
  id: int("id").autoincrement().primaryKey(),
  txType: mysqlEnum("txType", [
    "participant_payment",
    "settlement_to_organizer",
    "refund_to_participant",
    "platform_withdrawal",
    "platform_fee",
    "topup_received",
    "manual_adjustment",
...
// ─── Platform Wallet Balance（平台余额快照）────────────────────────────────
export const platformBalance = mysqlTable("platform_balance", {
  id: int("id").autoincrement().primaryKey(),
  balance: decimal("balance", { precision: 14, scale: 2 }).notNull().default("0.00"),
  pendingSettlement: decimal("pendingSettlement", { precision: 14, scale: 2 }).notNull().default("0.00"),
  totalCollected: decimal("totalCollected", { precision: 14, scale: 2 }).notNull().default("0.00"),
  totalSettled: decimal("totalSettled", { precision: 14, scale: 2 }).notNull().default("0.00"),
  totalRefunded: decimal("totalRefunded", { precision: 14, scale: 2 }).notNull().default("0.00"),
  totalWithdrawn: decimal("totalWithdrawn", { precision: 14, scale: 2 }).notNull().default("0.00"),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
export type PlatformBalance = typeof platformBalance.$inferSelect;
