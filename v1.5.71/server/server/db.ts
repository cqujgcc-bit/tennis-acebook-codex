...
export async function getPlatformFinanceOverview() {
  const db = await getDb();
  if (!db) return { totalCollected: 0, totalSettled: 0, totalPending: 0, totalRefunded: 0, totalWithdrawn: 0, currentBalance: 0, orderStats: { total: 0, paid: 0, settled: 0, refunded: 0 }, matchStats: { total: 0, completed: 0, settled: 0 } };
  const { matchOrders, matchSettlements, tennisMatches, platformBalance } = await import("../drizzle/schema");
  const { eq, sql } = await import("drizzle-orm");
  const allOrders = await db.select().from(matchOrders);
  const totalCollect = allOrders.filter(o => o.status === "paid" || o.status === "settled").reduce((s, o) => s + Number(o.amount), 0);
  const settledOrders = allOrders.filter(o => o.status === "settled").reduce((s, o) => s + Number(o.amount), 0);
  const refundedOrders = allOrders.filter(o => o.status === "refunded").reduce((s, o) => s + Number(o.amount), 0);
  const allSettlements = await db.select().from(matchSettlements);
  const settledSettlements = allSettlements.filter(s => s.status === "settled").reduce((sum, s) => sum + Number(s.netAmount), 0);
  const pendingSettlements = allSettlements.filter(s => s.status === "confirming").reduce((sum, s) => sum + Number(s.netAmount), 0);
  let currentBalance = 0;
  try {
    const [bal] = await db.select().from(platformBalance).limit(1);
    if (bal) currentBalance = Number(bal.balance);
  } catch (e) { /* ignore */ }
  return {
    totalCollected: Math.round(totalCollect * 100) / 100,
    totalSettled: Math.round(settledSettlements * 100) / 100,
    totalPending: Math.round(pendingSettlements * 100) / 100,
    totalRefunded: Math.round(refundedOrders * 100) / 100,
    totalWithdrawn: 0,
    currentBalance: Math.round(Math.max(0, totalCollect - settledSettlements - refundedOrders - settledSettlements) * 100) / 100,
    orderStats: { total: allOrders.length, paid: allOrders.filter(o => o.status === "paid").length, settled: allOrders.filter(o => o.status === "settled").length, refunded: allOrders.filter(o => o.status === "refunded").length },
    matchStats: { total: Number((await db.select({ count: sql\`count(*)\` }).from(tennisMatches))[0]?.count ?? 0), completed: Number((await db.select({ count: sql\`count(*)\` }).from(tennisMatches).where(eq(tennisMatches.status, "completed")))[0]?.count ?? 0), settled: allSettlements.filter(s => s.status === "settled").length },
  };
}
/** 获取所有结算记录 */
export async function getAllSettlements(limit = 50, offset = 0) {
  const db = await getDb(); if (!db) return [];
  const { matchSettlements, tennisMatches, users } = await import("../drizzle/schema");
  const { eq, desc } = await import("drizzle-orm");
  return db.select({
    matchId: matchSettlements.matchId, organizerId: matchSettlements.organizerId,
    totalAmount: matchSettlements.totalAmount, netAmount: matchSettlements.netAmount,
    platformFee: matchSettlements.platformFee, status: matchSettlements.status,
    confirmedAt: matchSettlements.confirmedAt, settledAt: matchSettlements.settledAt,
    wxBatchId: matchSettlements.wxBatchId, matchTitle: tennisMatches.title,
    matchDate: tennisMatches.matchDate, organizerName: users.name,
  }).from(matchSettlements)
    .leftJoin(tennisMatches, eq(matchSettlements.matchId, tennisMatches.id))
    .leftJoin(users, eq(matchSettlements.organizerId, users.id))
    .orderBy(desc(matchSettlements.createdAt)).limit(limit).offset(offset);
}
/** 记录平台交易流水 */
export async function recordPlatformTransaction(tx: {
  txType: string; amount: number; matchId?: number; orderId?: string;
  userId?: number; operatorId: number; description: string;
  wxBatchId?: string; wxTransactionId?: string;
}) {
  const db = await getDb(); if (!db) return;
  const { platformTransactions: ptx, platformBalance: pbal } = await import("../drizzle/schema");
  const { eq } = await import("drizzle-orm");
  let [balance] = await db.select().from(pbal).limit(1);
  if (!balance) {
    await db.insert(pbal).values({ balance: "0.00", pendingSettlement: "0.00", totalCollected: "0.00", totalSettled: "0.00", totalRefunded: "0.00", totalWithdrawn: "0.00" });
    [balance] = await db.select().from(pbal).limit(1);
  }
  const oldBalance = Number(balance.balance);
  const newBalance = oldBalance + tx.amount;
  await db.insert(ptx).values({
    txType: tx.txType as any, amount: String(tx.amount),
    balanceAfter: String(Math.round(newBalance * 100) / 100),
    matchId: tx.matchId, orderId: tx.orderId, userId: tx.userId,
    operatorId: tx.operatorId, description: tx.description,
    wxBatchId: tx.wxBatchId, wxTransactionId: tx.wxTransactionId,
  });
}
