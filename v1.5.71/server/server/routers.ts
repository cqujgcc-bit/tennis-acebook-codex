  ...
  admin: router({
    ...
    finance: router({
      overview: adminProcedure.query(async () => {
        return db.getPlatformFinanceOverview();
      }),
      settlements: adminProcedure
        .input(z.object({ limit: z.number().default(50), offset: z.number().default(0) }).optional())
        .query(async ({ input }) => {
          const { limit = 50, offset = 0 } = input || {};
          return db.getAllSettlements(limit, offset);
        }),
      transactions: adminProcedure
        ...
        .query(async ({ input }) => {
          ...
          const dbInst = await db.getDb();
          if (!dbInst) return [];
          const { platformTransactions: ptx } = await import("../drizzle/schema");
          const { desc: descOp } = await import("drizzle-orm");
          return dbInst.select().from(ptx).orderBy(descOp(ptx.createdAt)).limit(limit).offset(offset);
        }),
      withdraw: adminProcedure
        .input(z.object({ amount: z.number().positive(), description: z.string().min(1).max(200) }))
        .mutation(async ({ ctx, input }) => {
          const overview = await db.getPlatformFinanceOverview();
          if (input.amount > overview.currentBalance) {
            throw new TRPCError({ code: "BAD_REQUEST", message: `可提现余额不足。当前：¥${overview.currentBalance.toFixed(2)}，提现：¥${input.amount.toFixed(2)}` });
          }
          await db.recordPlatformTransaction({
            txType: "platform_withdrawal", amount: -input.amount,
            operatorId: ctx.user.id,
            description: `平台提现：${input.description}（操作人ID: ${ctx.user.id}）`,
          });
          return { success: true, amount: input.amount, message: `提现已记录：¥${input.amount.toFixed(2)}` };
        }),
    }),
