...
/** 微信会在收款页展示 transfer_remark；统一说明这是 AceBook 球局场地费。 */
export function buildMatchTransferRemark(title: unknown, kind: "settlement" | "topup" = "settlement"): string {
  const prefix = kind === "topup" ? "AceBook球局场地费补缴-" : "AceBook球局场地费结算-";
  const normalizedTitle = String(title || "球局").replace(/\s+/g, " ").trim() || "球局";
  return `${prefix}${normalizedTitle}`.slice(0, 32);
}
...
function buildTransferSceneReportInfos(sceneId: string, remark: string): Array<{ info_type: string; info_content: string }> {
  ...
  if (sceneId === "1005") {
    return [
      ...
      { info_type: "报酬说明", info_content: "球局场地费垫付分摊" },
    ];
  }
...
function buildUserRecvPerception(sceneId: string): string {
  ...
  // 场景 1005 官方允许的收款感知：劳务报酬（默认）、报销款、企业补贴、开工利是。
  // 球场费用属于发起人垫付后的分摊报销，主动传"报销款"，用户收款页不再展示"劳务报酬"。
  if (sceneId === "1005") return "报销款";
  return "";
}
...
export async function createPrepay(params: {
  ...
  return buildClientPaymentParams(resp.prepay_id);
}

/** Re-sign an existing prepay_id so payment retries reuse the same WeChat order. */
export function buildClientPaymentParams(prepayId: string) {
  const ts = getTimestamp();
  const nonce = generateNonce(16);
