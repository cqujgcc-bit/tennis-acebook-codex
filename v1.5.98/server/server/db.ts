...
export async function getTennisMatches(opts?: {
  ...
  try {
    ...
    // 历史查询优先取最近记录，避免 limit 截断后只返回最老的一批球局。
    if (opts?.dateTo && !opts?.dateFrom) {
      query.orderBy(desc(tennisMatches.matchDate), desc(tennisMatches.startTime));
    } else {
    ...
    if (!opts?.status && !opts?.includeCancelled) {
...
export async function getTennisMatchById(id: number): Promise<typeof tennisMatches.$inferSelect | null> {
  const db = await getDb();
  if (!db) return null;
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        circleOnly: (rows[0] as any).circleOnly ?? false,
        feeRequired: (rows[0] as any).feeRequired ?? false,
      } as typeof tennisMatches.$inferSelect;
    } catch (fallbackError) {
...
export async function joinTennisMatch(matchId: number, userId: number, message?: string, paymentStatus?: string) {
  ...
  await db.insert(matchParticipants).values(values).onDuplicateKeyUpdate({
    set: { status: "confirmed", message: message ?? null, ...(paymentStatus ? { paymentStatus } : {}) } as any,
  });
}
