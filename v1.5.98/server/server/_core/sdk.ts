...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    // Support both Cookie (web) and Authorization Bearer token (mini-program)
    ...
    // Phone/Email/WeChat-based login: openId starts with 'phone_', 'email_', or 'wx_' — skip OAuth sync
    const isLocalLogin = sessionUserId.startsWith("phone_") || sessionUserId.startsWith("email_") || sessionUserId.startsWith("wx_");

    // If user not in DB, sync from OAuth server automatically (Manus OAuth only).
    // When self-hosted without a Manus OAuth server configured, skip this entirely.
