...
import { sdk } from "./sdk";
import { getAffectedRows } from "./db-result";
...
async function startServer() {
  ...
  app.use("/api/wxpay/transfer-notify", express.raw({ type: "application/json", limit: "1mb" }));
  ...
  app.post("/api/upload", async (req, res) => {
    try {
      ...
      bb.on("finish", async () => {
        try {
          if (rejectedType) return res.status(415).json({ error: "仅允许 JPG、PNG、WebP 或 GIF 图片" });
  ...
  app.post("/api/scheduled/close-expired-matches", async (req, res) => {
    try {
      ...
      // 先锁定本次将被关闭的 ID，后续奖励只处理这一批，避免重复扫描历史 completed 球局。
      const expiringMatches = await db
        .select({ id: tennisMatches.id })
        .from(tennisMatches)
        .where(
          and(
            or(eq(tennisMatches.status, "open"), eq(tennisMatches.status, "full")),
            or(
              lt(tennisMatches.matchDate, todayCSTStr),
              and(eq(tennisMatches.matchDate, todayCSTStr), sql`${tennisMatches.startTime} <= ${currentTimeStr}`)
            )
          )
        );
      ...
      const closed = getAffectedRows(result);
      ...
      if (closed > 0) {
        try {
          ...
          const justClosedMatches = expiringMatches;
    ...
    try {
      ...
      for (const settlement of pendingSettlements) {
        try {
          ...
          const { queryTransferBill, transferToUser, generateOrderId, encodeTransferPackageInfo, buildMatchTransferRemark } = await import("../wxpay");
          const organizer = await dbMod.getUserById(settlement.organizerId);
          let shouldCreateNew = false;
          let settled = false;

          if (settlement.wxBatchId) {
            try {
              const q = await queryTransferBill(settlement.wxBatchId);
              console.log(`[auto-settle] match=${settlement.matchId} bill=${settlement.wxBatchId} state=${q.state}`);
              if (q.state === "SUCCESS") {
                settled = true;
              } else if (q.state === "FAIL" || q.state === "CANCELLED") {
                shouldCreateNew = true;
              }
            } catch (queryErr: any) {
              console.warn(`[auto-settle] query failed match=${settlement.matchId} bill=${settlement.wxBatchId}:`, queryErr?.message || queryErr);
              // 原单状态未知时不要换新单，避免重复转账
            }
          } else {
            shouldCreateNew = true;
          ...
          if (settled) {
          ...
          } else if (shouldCreateNew && organizer?.wechatOpenid && Number(settlement.netAmount) > 0) {
            const amountFen = Math.round(Number(settlement.netAmount) * 100);
            const transfer = await transferToUser({
              batchId: generateOrderId(),
              openid: organizer.wechatOpenid,
              amountFen,
              remark: buildMatchTransferRemark(`球局${settlement.matchId}`),
            });
            const isSuccess = transfer.status === "SUCCESS";
            await db.update(matchSettlements)
              .set({
                status: isSuccess ? "settled" : "confirming",
                settledAt: isSuccess ? now : null,
            ...
            if (isSuccess) {
              ...
              console.log(`[auto-settle] match=${settlement.matchId} still ${transfer.status}`);
            }
          } else {
            console.log(`[auto-settle] match=${settlement.matchId} kept confirming (no payment action)`);
          }
        } catch (innerErr: any) {
    ...
    try {
      ...
      if (isWxpayConfigured()) {
        ...
        if (pubKey) {
          ...
          console.error(`[wxpay/notify] WXPAY_PUBLIC_KEY not configured`);
          return res.status(503).json({ code: "CONFIG_ERROR", message: "支付回调验签未配置" });
        }
      }
      ...
      if (payResult.trade_state === "SUCCESS") {
        ...
        const db = await getDb();
        if (db) {
          const { matchOrders, matchParticipants, tennisMatches, matchMessages, circleActivities, circleActivitySignups } = await import("../../drizzle/schema");
          ...
          // pending -> paid 是回调的原子幂等锁；只有抢到的请求能执行后续资金逻辑。
          const claimed = await db.update(matchOrders)
            .set({ status: "paid", paidAt: new Date(), wxTransactionId: payResult.transaction_id })
            .where(and(eq(matchOrders.orderId, payResult.out_trade_no), eq(matchOrders.status, "pending")));
          if (getAffectedRows(claimed) === 0) {
            // v1.5.94 曾在订单已置为 paid 后误判 mysql2 affectedRows，导致参与者仍停在 pending。
            // 仅对这种「已付款预付单 + 待支付占位」继续补偿；活动、补缴和正常重复回调仍直接幂等返回。
            let shouldRecoverLegacyPrepay = false;
            if (order.status === "paid" && order.matchId > 0 && (order as any).orderType === "prepay") {
              const pendingParticipants = await db.select({ id: matchParticipants.id })
                .from(matchParticipants)
                .where(and(
                  eq(matchParticipants.matchId, order.matchId),
                  eq(matchParticipants.userId, order.userId),
                  eq(matchParticipants.status, "pending"),
                ))
                .limit(1);
              shouldRecoverLegacyPrepay = pendingParticipants.length > 0;
            }
            if (!shouldRecoverLegacyPrepay) {
              return res.status(200).json({ code: "SUCCESS", message: "成功" });
            }
            console.warn(`[wxpay/notify] 恢复历史已付款待入局订单 order=${order.orderId}`);
          }
          {
            // 负 matchId 表示圈子活动 AA 订单。
            if (order.matchId < 0) {
              const activityId = -order.matchId;
              await db.update(circleActivitySignups)
                .set({ payStatus: "paid", paidAt: new Date() })
                .where(and(eq(circleActivitySignups.activityId, activityId), eq(circleActivitySignups.userId, order.userId), eq(circleActivitySignups.orderId, order.orderId)));
              const remaining = await db.select({ id: circleActivitySignups.id }).from(circleActivitySignups)
                .where(and(eq(circleActivitySignups.activityId, activityId), eq(circleActivitySignups.payStatus, "unpaid")));
              if (remaining.length === 0) {
                await db.update(circleActivities).set({ settleStatus: "settled", settledAt: new Date() })
                  .where(eq(circleActivities.id, activityId));
              }
              return res.status(200).json({ code: "SUCCESS", message: "成功" });
            }
              ...
              try {
                ...
                if (matchRowsT.length > 0) {
                  ...
                  if (organizer?.wechatOpenid) {
                    const { transferToUser, buildMatchTransferRemark } = await import("../wxpay");
                    // 使用订单号派生固定转账单号，即使进程中断也不会生成第二笔转账。
                    const batchId = `TU${order.orderId}`.replace(/[^A-Za-z0-9_-]/g, "").slice(0, 32);
                    const transfer = await transferToUser({ batchId, openid: organizer.wechatOpenid, amountFen: Math.round(Number(order.amount) * 100), remark: buildMatchTransferRemark(mt.title, "topup") });
                    if (transfer.status === "SUCCESS") {
                      ...
                      const notice = transfer.status === "WAIT_USER_CONFIRM"
                        ? `「${mt.title}」补缴 ${Number(order.amount).toFixed(2)} 元待您在微信确认收款。`
                        : transfer.status === "FAIL"
                          ? `「${mt.title}」补缴 ${Number(order.amount).toFixed(2)} 元结算失败，请联系平台处理。`
                          : `「${mt.title}」补缴 ${Number(order.amount).toFixed(2)} 元结算处理中，请稍后查看钱包。`;
                      await dbMod.createNotification({ userId: mt.authorId, type: "system", title: "补缴结算", content: notice });
                    }
                  }
                }
              } catch (_topupErr) {
            ...
            // 原子地从 pending 转 confirmed。回调和主动查单并发时也只允许一方占位。
            const participantClaim = await db.update(matchParticipants)
            ...
            const wasPending = getAffectedRows(participantClaim) > 0;
  ...
  // ── 商家转账结果回调：只有微信确认 SUCCESS 后才将球局标记为已结算 ────────
  app.post("/api/wxpay/transfer-notify", async (req, res) => {
    try {
      const rawBody = Buffer.isBuffer(req.body) ? req.body.toString("utf8") : JSON.stringify(req.body);
      const body = JSON.parse(rawBody) as { resource?: { ciphertext: string; nonce: string; associated_data: string } };
      if (!body.resource) return res.status(400).json({ code: "PARAM_ERROR", message: "缺少 resource" });
      const { verifyNotifySignature, getVerifyPublicKey, decryptNotifyData, isWxpayConfigured } = await import("../wxpay");
      if (isWxpayConfigured()) {
        const key = getVerifyPublicKey();
        if (key) {
          const valid = verifyNotifySignature(String(req.headers["wechatpay-timestamp"] || ""), String(req.headers["wechatpay-nonce"] || ""), rawBody, String(req.headers["wechatpay-signature"] || ""), key);
          if (!valid) return res.status(401).json({ code: "SIGN_ERROR", message: "签名验证失败" });
        } else {
          console.error("[wxpay/transfer-notify] WXPAY_PUBLIC_KEY not configured");
          return res.status(503).json({ code: "CONFIG_ERROR", message: "转账回调验签未配置" });
        }
      }
      const result = JSON.parse(decryptNotifyData(body.resource.ciphertext, body.resource.nonce, body.resource.associated_data)) as { out_bill_no: string; state: string };
      const { getDb } = await import("../db");
      const db = await getDb();
      if (db && result.state === "SUCCESS") {
        const { matchSettlements, matchOrders } = await import("../../drizzle/schema");
        const { eq, and, inArray } = await import("drizzle-orm");
        const rows = await db.select().from(matchSettlements).where(eq(matchSettlements.wxBatchId, result.out_bill_no));
        for (const settlement of rows) {
          await db.update(matchSettlements).set({ status: "settled", settledAt: new Date() }).where(eq(matchSettlements.id, settlement.id));
          await db.update(matchOrders).set({ status: "settled", settledAt: new Date() }).where(and(eq(matchOrders.matchId, settlement.matchId), inArray(matchOrders.status, ["paid"])));
        }
      }
      res.status(200).json({ code: "SUCCESS", message: "成功" });
    } catch (err: any) {
      console.error("[wxpay/transfer-notify] Error:", err?.message);
  ...
  server.listen(port, async () => {
    ...
    // 结算兜底由已鉴权的 /api/scheduled/auto-settle-matches 定时调用，
    // 不再加载不存在的本地 settlement-cron 模块。
  });
}
