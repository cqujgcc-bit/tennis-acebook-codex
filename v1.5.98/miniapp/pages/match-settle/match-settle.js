...
Page({
  ...
  load: function (id) {
    ...
    api.match.getById(id).then(function (res) {
      ...
      var settlementStatus = (res && res.settlementStatus) || '';
      ...
      that.setData({
        loading: false,
        alreadyCompleted: m.status === 'completed',
        completionMessage: m.status === 'completed'
          ? (settlementStatus === 'settled' ? '该球局已完成结算。'
            : (settlementStatus === 'confirming' ? '该球局已提交结算，请返回详情或钱包查看到账状态。'
              : (settlementStatus === 'disputed' ? '该球局结算存在异议，请到钱包查看。' : '该球局已结束，无需结算。')))
          : '',
        title: m.title || '球局结算',
  ...
  onToggle: function (e) {
    if (this.data.alreadyCompleted) return;
  ...
  onSubmit: function () {
    if (this.data.submitting || this.data.alreadyCompleted) return;
    var that = this;
    var list = this.data.payList;
    var organizerAttended = this.data.organizerAttended;
    if (list.length === 0) {
  ...
  doConfirm: function (attendance, organizerAttended) {
    ...
    api.match.confirmComplete(matchId, attendance, organizerAttended).then(function (r) {
      ...
      that.setData({
        ...
        completionMessage: (r && r.message) || '结算已提交，请查看到账状态。'
      });
      that.refreshPreviousPage();
      if (r && r.transfer && r.transfer.status === 'WAIT_USER_CONFIRM' && r.transfer.packageInfo) {
        if (!wx.canIUse('requestMerchantTransfer')) {
          wx.showModal({ content: '当前微信版本不支持确认收款，请更新微信后重试。', showCancel: false });
          return;
        }
        wx.requestMerchantTransfer({
          mchId: r.transfer.mchId,
          appId: wx.getAccountInfoSync().miniProgram.appId,
          package: r.transfer.packageInfo,
          success: function () {
            // JSAPI success 只表示成功打开收款页，最终到账以微信转账单 SUCCESS 为准。
            wx.showToast({ title: '已返回收款页，到账结果将自动同步', icon: 'none', duration: 3000 });
            that.refreshPreviousPage();
            setTimeout(function () { wx.navigateBack(); }, 1500);
          },
          fail: function (err) {
            var msg = (err && err.errMsg) || '';
            wx.showToast({ title: msg.indexOf('cancel') >= 0 ? '您已取消收款确认' : '未能打开收款确认页', icon: 'none' });
          }
        });
        return;
      }
